...
ES8311_DAC_REG34        = const(0x34)
ES8311_DAC_REG35        = const(0x35)
...
class ES8311:
    ...
    def setSpkVolume(self, vol):
        ...
        if vol > 100:
            vol = 100
        # store volume for later use (e.g. after reset)
        self._spk_volume = vol
        self._setSpkVolumeRegister()

    def _setSpkVolumeRegister(self):
        if self.debug:
            print("ES8311: setting speaker volume register")
            print("   to %d%%" % self._spk_volume)
            
        reg = 0 if self._spk_volume == 0 else ((self._spk_volume * 256) // 100) - 1
        # reg34,35 from pyramid code
        self.write_reg(ES8311_DAC_REG34, 0x80)
        self.write_reg(ES8311_DAC_REG35, 0x90)
    ...
    def start(self, record=False):
        ...
        """
        if self.debug:
            print("ES8311: starting %s" % ("recording" if record else "playback"))
        ...
        if self.debug:
            print("ES8311: power-up sequence done")
